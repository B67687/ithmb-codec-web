...
describe("act port assignment regression guard", () => {
  it("each browser gets a distinct act port", () => {
    const ports = Object.values(ACT_PORTS);
    expect(new Set(ports).size).toBe(ports.length);
  });

  it("no act port collides with the local-dev preview port 8899", () => {
    for (const port of Object.values(ACT_PORTS)) {
      expect(port).not.toBe(8899);
    }
  });

  it("all act ports fall in the reserved range", () => {
    for (const port of Object.values(ACT_PORTS)) {
  ...
  it("act resolution maps each browser to its dedicated port", () => {
  ...
  it("unknown browser under act falls back to the chromium port", () => {
    expect(resolveWebPort(true, undefined)).toBe(8999);
    expect(resolveWebPort(true, "edge")).toBe(8999);
  });

  it("non-act (local dev and real CI) keeps the 8899 reuse flow", () => {
